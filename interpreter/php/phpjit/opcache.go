...
func (i *opcacheInstance) Detach(ebpf interpreter.EbpfHandler, pid libpf.PID) error {
	...
	for _, prefix := range i.prefixes {
		if err2 := ebpf.DeletePidInterpreterMapping(pid, prefix); err2 != nil {
			err = multierr.Append(err, fmt.Errorf("failed to remove page 0x%x/%d: %w",
...
func (i *opcacheInstance) SynchronizeMappings(ebpf interpreter.EbpfHandler,
	...
	data := C.PHPJITProcInfo{
		start: C.u64(dasmBuf),
		end:   C.u64(dasmBuf + dasmSize),
	}

	pid := pr.PID()
	if err = ebpf.UpdateProcData(libpf.PHPJIT, pid, unsafe.Pointer(&data)); err != nil {
		return err
	}
...
func determineOPCacheVersion(ef *pfelf.File) (uint, error) {
	...
	if versionString == "" || !libpf.IsValidString(versionString) {
	...
	return php.VersionExtract(versionString)
}
...
func getOpcacheJITInfo(ef *pfelf.File) (dasmBuf, dasmSize libpf.Address, err error) {
	...
	dasmBufPtr, dasmSizePtr, err := php.RetrieveJITBufferPtrWrapper(code, zendJit)
	if err != nil {
