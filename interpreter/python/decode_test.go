...
func TestAnalyzeArm64Stubs(t *testing.T) {
	...
	// Python 3.12, static build with ADRP+ADD forming _PyRuntime directly
	// PyGILState_GetThisThreadState at 0x2204cc, _PyRuntime at 0x60c818
	val = decodeStubArgumentARM64(
		[]byte{
			0x5f, 0x24, 0x03, 0xd5, // bti c
			0x60, 0x1f, 0x00, 0x90, // adrp x0, .+0x3ec000
			0x00, 0x60, 0x20, 0x91, // add  x0, x0, #0x818
			0x01, 0x08, 0x46, 0xb9, // ldr  w1, [x0, #1544]
			0x61, 0x00, 0x00, 0x34, // cbz  w1, .+0xc
			0x00, 0x0c, 0x46, 0xb9, // ldr  w0, [x0, #1548]
			0x3b, 0x78, 0xfb, 0x17, // b    pthread_getspecific
			0x00, 0x00, 0x80, 0xd2, // mov  x0, #0x0
			0xc0, 0x03, 0x5f, 0xd6, // ret
		},
		0x60c818, 0x2204cc)
	// _PyRuntime(0x60c818) + 0x60c = 0x60ce24
	assert.Equal(t, libpf.SymbolValue(0x60ce24), val,
		"PyGILState_GetThisThreadState with direct ADRP+ADD")

	// Python 3.12, static build with ADRP+LDR (no ADD intermediate)
	// PyGILState_GetThisThreadState at 0x14d5b0, _PyRuntime at 0x6ccbd0
	val = decodeStubArgumentARM64(
		[]byte{
			0x08, 0x2c, 0x00, 0x90, // adrp x8, .+0x580000
			0x08, 0xd9, 0x41, 0xb9, // ldr  w8, [x8, #472]
			0x88, 0x00, 0x00, 0x34, // cbz  w8, .+0x10
			0x08, 0x2c, 0x00, 0x90, // adrp x8, .+0x580000
			0x00, 0xdd, 0x41, 0xb9, // ldr  w0, [x8, #476]
			0xc3, 0xc2, 0xfe, 0x17, // b    pthread_getspecific
			0xe0, 0x03, 0x1f, 0xaa, // mov  x0, xzr
			0xc0, 0x03, 0x5f, 0xd6, // ret
		},
		0x6ccbd0, 0x14d5b0)
	// _PyRuntime(0x6ccbd0) + 0x60c = 0x6cd1dc
	assert.Equal(t, libpf.SymbolValue(0x6cd1dc), val,
		"PyGILState_GetThisThreadState with direct ADRP+LDR")
}
