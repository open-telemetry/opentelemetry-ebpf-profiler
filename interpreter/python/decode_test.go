...
func TestExtractTLSOffsetFromCodeAMD64(t *testing.T) {
	testdata := []struct {
		name          string
		code          []byte
		baseAddr      uint64
	...
	for _, td := range testdata {
		t.Run(td.name, func(t *testing.T) {
			offset, err := extractTLSOffsetFromCodeAMD64(td.code, td.baseAddr)
			if td.expectedError != "" {
				require.Error(t, err)
				require.Contains(t, err.Error(), td.expectedError)
			} else {
...
func TestExtractTLSOffsetFromCodeARM64(t *testing.T) {
	...
	for _, td := range testdata {
		t.Run(td.name, func(t *testing.T) {
			...
			offset, err := extractTLSOffsetFromCodeARM64(td.code, td.baseAddr, visited, 0, nil)
			if td.expectedError != "" {
				require.Error(t, err)
				require.Contains(t, err.Error(), td.expectedError)
			} else {
