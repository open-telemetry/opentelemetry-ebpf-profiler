...
func (emc *ebpfMapsCoredump) UpdateProcData(t libpf.InterpreterType, pid libpf.PID,
	...
	case libpf.GoLabels:
		emc.ctx.addMap(unsafe.Pointer(&C.go_labels_procs), C.u32(pid),
			sliceBuffer(ptr, C.sizeof_GoLabelsOffsets))
	}
	return nil
}
...
func (emc *ebpfMapsCoredump) DeleteProcData(t libpf.InterpreterType, pid libpf.PID) error {
	...
	case libpf.GoLabels:
		emc.ctx.delMap(unsafe.Pointer(&C.go_labels_procs), C.u32(pid))
	}
	return nil
}
