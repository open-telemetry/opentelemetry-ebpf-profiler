...
func newRebaseCmd(store *modulestore.Store) *ffcli.Command {
	...
	return &ffcli.Command{
		...
		ShortHelp: "Update all test cases (or a subset of them) by running them and" +
			" saving the current unwinding",
		FlagSet: set,
	}
}
...
func (cmd *rebaseCmd) exec(context.Context, []string) (err error) {
	...
	for _, testCasePath := range cases {
		...
		testCase.Threads, err = ExtractTracesWithInterpreters(context.Background(),
			core, false, nil, nil, testCase.interpretersConfig())
		_ = core.Close()
		if err != nil {
