...
func (d *hotspotInstance) getMethod(addr libpf.Address, _ uint32) (*hotspotMethod, error) {
	...
	if sourceFileName == libpf.NullString {
		...
		sourceFileName = plugins.UnknownSourceFile
...
func (d *hotspotInstance) Detach(ebpf plugins.EbpfHandler, pid libpf.PID) error {
	var err error
...
func (d *hotspotInstance) addJitArea(ebpf plugins.EbpfHandler,
...
func (d *hotspotInstance) populateMainMappings(vmd *hotspotVMData,
	ebpf plugins.EbpfHandler, pid libpf.PID,
) error {
	if d.mainMappingsInserted {
...
func (d *hotspotInstance) SynchronizeMappings(ebpf plugins.EbpfHandler,
