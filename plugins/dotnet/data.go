...
type dotnetData struct {
	...
	walkRangeSectionsMethod func(i *dotnetInstance, ebpf plugins.EbpfHandler,
		pid libpf.PID) error
...
func (d *dotnetData) Attach(ebpf plugins.EbpfHandler, pid libpf.PID, bias libpf.Address,
	rm remotememory.RemoteMemory, cfg plugins.Config,
) (plugins.Instance, error) {
	if cfg.(plugins.DotnetConfig).IsDisabled() {
		return nil, plugins.ErrPluginDisabled
	}

	log.Debugf("Attach PID %d, bias %x", pid, bias)

	addrToMethod, err := freelru.New[libpf.Address, *dotnetMethod](plugins.LruFunctionCacheSize,
...
func (d *dotnetData) Unload(_ plugins.EbpfHandler) {
}
