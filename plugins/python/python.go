...
package python // import "go.opentelemetry.io/ebpf-profiler/plugins/python"
...
var _ plugins.Data = &pythonData{}

func (d *pythonData) String() string {
	return fmt.Sprintf("Python %d.%d", d.version>>8, d.version&0xff)
}

func (d *pythonData) Attach(_ plugins.EbpfHandler, _ libpf.PID, bias libpf.Address, rm remotememory.RemoteMemory, cfg plugins.Config) (plugins.Instance, error) {
	if cfg.(plugins.PythonConfig).IsDisabled() {
		return nil, plugins.ErrPluginDisabled
	}

	addrToCodeObject, err := freelru.New[libpf.Address, *pythonCodeObject](plugins.LruFunctionCacheSize,
...
func (d *pythonData) Unload(_ plugins.EbpfHandler) {
}
...
var _ plugins.Instance = &pythonInstance{}
...
func (p *pythonInstance) UpdateLibcInfo(ebpf plugins.EbpfHandler, pid libpf.PID,
...
func (p *pythonInstance) Detach(ebpf plugins.EbpfHandler, pid libpf.PID) error {
	if !p.procInfoInserted {
		return nil
	}
...
func findInterpreterRanges(info *plugins.LoaderInfo, ef *pfelf.File,
