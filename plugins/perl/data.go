...
package perl // import "go.opentelemetry.io/ebpf-profiler/plugins/perl"
...
func (d *perlData) Attach(_ plugins.EbpfHandler, _ libpf.PID, bias libpf.Address,
	rm remotememory.RemoteMemory, cfg plugins.Config,
) (plugins.Instance, error) {
	if cfg.(plugins.PerlConfig).IsDisabled() {
		return nil, plugins.ErrPluginDisabled
	}

	addrToHEK, err := freelru.New[libpf.Address, libpf.String](plugins.LruFunctionCacheSize,
		libpf.Address.Hash32)
	if err != nil {
		return nil, err
	}

	addrToCOP, err := freelru.New[copKey, *perlCOP](plugins.LruFunctionCacheSize*8,
		hashCOPKey)
	if err != nil {
		return nil, err
	}
...
func (d *perlData) Unload(_ plugins.EbpfHandler) {
}

func newData(ebpf plugins.EbpfHandler, info *plugins.LoaderInfo,
	ef *pfelf.File,
) (*perlData, error) {
