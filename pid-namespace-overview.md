...
|---|---------|---------------------|-----------------------------------|------------------------------------------------------|--------------------------------------------------------|----------------------------------------|
| **1** | **eBPF reports PIDs from kernel-root namespace, but profiler's `/proc` view is in a nested PID namespace** | Profiler in nested PID namespace (kind/minikube node, `hostPID: true` or container) | ❌ None — PIDs mismatch, process lookup fails | ✅ **Solves**: Matches `target_pid_ns_inode` to find target PID in profiler's namespace; safely filters sibling containers | ⚠️ **Partial**: Indexes `numbers[level]` without checking `inum`, causing PID collisions between sibling containers | ⚠️ **Partial**: Discovers host PID via `BPF_PROG_TEST_RUN`; uses `RootFs` to access host `/proc` |
| **2** | **Targets in descendant/child PID namespaces relative to profiler** | Kind/minikube DaemonSets, nested containers, container sidecars | ⚠️ `PIDNamespaceTranslation=true` (exact match only via `bpf_get_ns_current_pid_tgid`) | ✅ **Solves**: Walks task's `struct pid` hierarchy matching `target_pid_ns_inode`; supports `auto`/`exact`/`descendants` modes | ❌ Doesn't address (assumes profiler in nested ns) | ❌ Doesn't address |
