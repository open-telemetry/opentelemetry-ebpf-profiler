...
package golabels // import "go.opentelemetry.io/ebpf-profiler/extensions/golabels"
...
func (d *data) Attach(ebpf extensions.EbpfHandler, pid libpf.PID,
	_ libpf.Address, _ remotememory.RemoteMemory, cfg extensions.Config,
) (extensions.Instance, error) {
	if cfg.(extensions.LabelsConfig).IsDisabled() {
...
func (d *data) Detach(ebpf extensions.EbpfHandler, pid libpf.PID) error {
	return ebpf.DeleteProcData(libpf.GoLabels, pid)
}

func (d *data) Unload(_ extensions.EbpfHandler) {}
