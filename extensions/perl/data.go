...
package perl // import "go.opentelemetry.io/ebpf-profiler/extensions/perl"
...
func (d *perlData) Attach(_ extensions.EbpfHandler, _ libpf.PID, bias libpf.Address,
	rm remotememory.RemoteMemory, cfg extensions.Config,
) (extensions.Instance, error) {
	if cfg.(extensions.PerlConfig).IsDisabled() {
		return nil, extensions.ErrExtensionDisabled
	}

	addrToHEK, err := freelru.New[libpf.Address, libpf.String](extensions.LruFunctionCacheSize,
		libpf.Address.Hash32)
	if err != nil {
		return nil, err
	}

	addrToCOP, err := freelru.New[copKey, *perlCOP](extensions.LruFunctionCacheSize*8,
		hashCOPKey)
	if err != nil {
		return nil, err
	}
...
func (d *perlData) Unload(_ extensions.EbpfHandler) {
}

func newData(ebpf extensions.EbpfHandler, info *extensions.LoaderInfo,
	ef *pfelf.File,
) (*perlData, error) {
