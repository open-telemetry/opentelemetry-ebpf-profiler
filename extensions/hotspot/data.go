...
package hotspot // import "go.opentelemetry.io/ebpf-profiler/extensions/hotspot"
...
func (d *hotspotData) Attach(_ extensions.EbpfHandler, _ libpf.PID, bias libpf.Address,
	rm remotememory.RemoteMemory, cfg extensions.Config,
) (ii extensions.Instance, err error) {
	if cfg.(extensions.HotspotConfig).IsDisabled() {
	...
	addrToSymbol, err := freelru.New[libpf.Address, libpf.String](2*extensions.LruFunctionCacheSize,
		libpf.Address.Hash32)
	if err != nil {
		return nil, err
	}
	addrToMethod, err := freelru.New[libpf.Address, *hotspotMethod](extensions.LruFunctionCacheSize,
		libpf.Address.Hash32)
	if err != nil {
		return nil, err
	}
	addrToJITInfo, err := freelru.New[libpf.Address, *hotspotJITInfo](extensions.LruFunctionCacheSize,
		libpf.Address.Hash32)
	if err != nil {
		return nil, err
	}
...
func (d *hotspotData) Unload(_ extensions.EbpfHandler) {
}
...
func newHotspotData(filename string, ef *pfelf.File) (extensions.Data, error) {
	d := &hotspotData{}
