...
type dotnetData struct {
	...
	walkRangeSectionsMethod func(i *dotnetInstance, ebpf extensions.EbpfHandler,
		pid libpf.PID) error
...
func (d *dotnetData) Attach(ebpf extensions.EbpfHandler, pid libpf.PID, bias libpf.Address,
	rm remotememory.RemoteMemory, cfg extensions.Config,
) (extensions.Instance, error) {
	if cfg.(extensions.DotnetConfig).IsDisabled() {
		return nil, extensions.ErrExtensionDisabled
	}

	log.Debugf("Attach PID %d, bias %x", pid, bias)

	addrToMethod, err := freelru.New[libpf.Address, *dotnetMethod](extensions.LruFunctionCacheSize,
...
func (d *dotnetData) Unload(_ extensions.EbpfHandler) {
}
