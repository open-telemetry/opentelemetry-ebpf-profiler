...
// NewMultiDataWithConfigs creates a new MultiData instance with associated configs.
func NewMultiDataWithConfigs(interpreters []Data, configs []Config) *MultiData {
	return &MultiData{
		interpreters: interpreters,
		configs:      configs,
	}
...
func (m *MultiData) Attach(ebpf EbpfHandler, pid libpf.PID, bias libpf.Address,
	rm remotememory.RemoteMemory, cfg Config,
) (Instance, error) {
	var instances []Instance
	var errs []error

	for i, data := range m.interpreters {
		var dataCfg Config = cfg
		if i < len(m.configs) && m.configs[i] != nil {
			dataCfg = m.configs[i]
		}
		instance, err := data.Attach(ebpf, pid, bias, rm, dataCfg)
		if err != nil {
