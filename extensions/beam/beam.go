...
package beam // import "go.opentelemetry.io/ebpf-profiler/extensions/beam"
...
var (
	...
	_         extensions.Data     = &beamData{}
	_         extensions.Instance = &beamInstance{}
)
...
func Loader(ebpf extensions.EbpfHandler, info *extensions.LoaderInfo) (extensions.Data, error) {
...
func (d *beamData) Attach(ebpf extensions.EbpfHandler, pid libpf.PID, bias libpf.Address, rm remotememory.RemoteMemory, cfg extensions.Config) (extensions.Instance, error) {
	if cfg.(extensions.BEAMConfig).IsDisabled() {
	...
	atomCache, err := freelru.New[uint32, libpf.String](
		extensions.LruFunctionCacheSize, hash.Uint32)
	if err != nil {
		return nil, err
	}

	mfaNameCache, err := freelru.New[beamMfa, libpf.String](
		extensions.LruFunctionCacheSize, hashMFA)
	if err != nil {
		return nil, err
	}

	stringCache, err := freelru.New[libpf.Address, libpf.String](
		extensions.LruFunctionCacheSize, libpf.Address.Hash32)
	if err != nil {
		return nil, err
	}
...
func (d *beamData) Unload(_ extensions.EbpfHandler) {
}

func (i *beamInstance) SynchronizeMappings(ebpf extensions.EbpfHandler, _ reporter.ExecutableReporter, pr process.Process, mappings []process.RawMapping) error {
	pid := pr.PID()
	i.mappingGeneration++
...
func (i *beamInstance) Detach(extensions.EbpfHandler, libpf.PID) error {
	return nil
}
