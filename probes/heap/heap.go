...
func (hp *Probe) Load(_ context.Context, reg tracer.ProbeRegistrar, pctx *tracer.ProbeContext) error {
	...
	hp.originAlloc, err = reg.Register(&samples.TypeMetadata{
		...
		SampleUnit: "bytes",
		// The eBPF side populates both slots: [0] the allocation pointer,
...
func (hp *Probe) Attach(pr process.Process, mapping *process.RawMapping,
	...
	// sites; Attach may also be retried for the same mapping, so dedup
	// by (fileID, name, offset).
...
func (hp *Probe) PreHandleTrace(trace *libpf.EbpfTrace) bool {
	if trace.Origin == hp.originFree {
		if hp.tracker != nil {
			hp.tracker.HandleFree(trace.PID, trace.ValueExtra[0])
		}
		return false // consumed
	}
	// Alloc event: stash raw fields for PostHandleTrace.
	hp.pendingAllocPID = trace.PID
	hp.pendingAllocPtr = trace.ValueExtra[0]
...
var inuseProfileType = &samples.TypeMetadata{
	...
	DerivedProfiles: []samples.DerivedProfile{{
		...
		SampleUnit: "count",
		Value:      inuseObjectsValue,
	}},
}

// inuseObjectsValue surfaces the live object count that ProduceSnapshots stores in
// ValueExtra[0]. Unlike allocObjectsValue this derives nothing: the live heap
// tracker maintains Space and Objects independently, so the count is carried
// alongside the value rather than computed from it.
func inuseObjectsValue(_ int64, extra [2]uint64) int64 {
...
func (hp *Probe) ProduceSnapshots() []samples.SnapshotProfile {
	...
	for i, e := range entries {
		result[i] = samples.SnapshotSample{
			...
			Frames:    e.Frames,
			Value:     e.Space,
			// ValueExtra[0] carries the live object count; see
			// inuseObjectsValue.
			ValueExtra: [2]uint64{uint64(e.Objects), 0},
		}
	}
