...
func Resolve(ef *pfelf.File, sym *libpf.Symbol) (Var, error) {
	...
	// relocations do not describe it. The real test is st_shndx == SHN_UNDEF, but
	// libpf.Symbol does not carry the section index, so we infer it from zero value
	// and size. Undefined symbols always have both zero, and a defined symbol with
	// zero address and zero size is rare enough that the two are almost always
	// equivalent.
