...
func TestLocate(t *testing.T) {
	tests := map[string]struct {
		...
		"general-dynamic": {
			...
			want: VarLocation{ModuleID: 3, Offset: 0x40},
		},
		...
		"local-dynamic": {
			...
			want: VarLocation{ModuleID: 5, Offset: 0x18},
		},
		"tlsdesc unrelocated": {
		...
		"tlsdesc dynamic on x86-64": {
			...
			want: VarLocation{ModuleID: 2, Offset: 0x30},
		},
		...
		"tlsdesc tls_index with an implausible module": {
			...
			mem: memory{
				slotAddr:  words(resolverAddr, indexAddr),
				indexAddr: words(math.MaxUint16+1, 0x30),
			},
			wantAnyErr: true,
		},
		"tlsdesc static on aarch64": {
			...
			want: VarLocation{Offset: lowIndexAddr},
		},
		"tlsdesc local-dynamic on aarch64": {
			...
			want: VarLocation{ModuleID: 4, Offset: 0x58},
		},
