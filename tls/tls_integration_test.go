	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			for varName, rep := range reports {
				...
				loc, err := v.Locate(rm, bias)
				require.NoError(t, err)

				require.Equal(t, tc.wantDynamic, loc.ModuleID != 0,
					"%s resolved to the wrong kind of TLS", varName)
				if loc.ModuleID != 0 {
					require.NotZero(t, dtv.Multiplier, "no DTV layout extracted from libc")
				}

				info, err := loc.VarInfo(dtv)
				require.NoError(t, err)

				addr, err := varAddress(rm, info, rep.tpBase)
				require.NoError(t, err)
...
func varAddress(rm remotememory.RemoteMemory, v support.TLSVarInfo,
	tpBase uint64) (libpf.Address, error) {
	// Signed: variant II puts the static TLS block below the thread pointer.
	offset := libpf.Address(int64(v.Tls_offset))
	if v.Dtv_pos == 0 {
		return libpf.Address(tpBase) + offset, nil
	}

	dtvPtr, err := rm.ReadPtr(libpf.Address(tpBase + uint64(int64(v.Dtv_offset))))
	if err != nil {
		return 0, err
	}
	block, err := rm.ReadPtr(dtvPtr + libpf.Address(v.Dtv_pos))
	if err != nil {
		return 0, err
	}
