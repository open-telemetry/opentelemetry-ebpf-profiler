...
// file, and Locate turns that into a VarLocation, once per process.
...
var (
	...
	// ErrNotThreadLocal means the symbol is not a thread-local this file
	// defines, so its relocations say nothing about where the variable lives.
	// Callers looking a published name up in an arbitrary file get this for one
	// that merely references the variable, or defines something else by that
	// name.
	ErrNotThreadLocal = errors.New("not a thread-local defined by this file")
)
...
//
// Turning the location into a descriptor eBPF can follow additionally needs the
// process libc's DTV layout when the variable is in dynamic TLS (VarInfo).
func (v Var) Locate(rm remotememory.RemoteMemory, bias libpf.Address) (VarLocation, error) {
	switch v.access {
	case accessLocalExec:
		return staticLoc(v.addend)
	...
	case accessInitialExec:
		...
		if got == 0 && v.variant == variantII {
			return VarLocation{}, fmt.Errorf("%w: TLS GOT slot", ErrUnresolved)
		}
		return staticLoc(got + v.addend)
	...
	case accessGeneralDynamic:
		...
		return dynamicLoc(moduleID, offset+v.addend)
	...
	case accessLocalDynamic:
		...
		return dynamicLoc(moduleID, v.addend)
	...
	case accessTLSDesc:
		...
		if resolver == 0 {
			return VarLocation{}, fmt.Errorf("%w: TLSDESC descriptor", ErrUnresolved)
		}
		return v.locateTLSDesc(rm, resolver, arg)

	default:
		return VarLocation{}, fmt.Errorf("%w: %v", ErrUnsupportedModel, v.access)
	}
}
...
func (v Var) locateTLSDesc(rm remotememory.RemoteMemory, resolver, arg uint64,
	...
	case variantII:
		...
		if int64(arg) < 0 {
			return staticLoc(arg + v.addend)
		}
		ti, err := derefTLSIndex(rm, arg)
		if err != nil {
			return VarLocation{}, fmt.Errorf("TLSDESC argument %#x is neither a negative "+
				"TP offset nor a tls_index: %w", arg, err)
		}
		return dynamicLoc(ti.moduleID, ti.offset+v.addend)
	...
	case variantI:
		...
		ti, err := derefTLSIndex(rm, arg)
		if err != nil {
			return VarLocation{}, fmt.Errorf("TLSDESC argument %#x is not a tls_index "+
				"and resolver %#x is not the static form: %w", arg, resolver, err)
		}
		return dynamicLoc(ti.moduleID, ti.offset+v.addend)
	}
	return VarLocation{}, fmt.Errorf("%w: TLS variant unset", ErrUnsupportedModel)
}
...
func derefTLSIndex(rm remotememory.RemoteMemory, addr uint64) (tlsIndex, error) {
	...
	// Same module bound dynamicLoc enforces, checked here to name the address
	// the pair came from.
