...
func parseBTF(vars *SysConfigVars, needTPBase, needProcessStartTime bool,
	...
	if needPIDNamespaceLayout {
		if err := parsePIDNamespaceLayout(spec, &vars.pid_namespace_layout); err != nil {
			...
			vars.translate_descendant_pids = true
		}
	}
...
func determineSysConfig(coll *cebpf.CollectionSpec, maps map[string]*cebpf.Map,
	...
	if vars.translate_descendant_pids {
		if err := determineTargetPIDNamespaceLevel(coll, maps, vars); err != nil {
			...
			vars.translate_descendant_pids = false
...
func pidNamespaceTranslationMode(cfg *Config) PIDNamespaceTranslationMode {
	if !cfg.PIDNamespaceTranslation {
		return PIDNamespaceTranslationModeNone
	}
	if cfg.PIDNamespaceTranslationMode == PIDNamespaceTranslationModeNone {
		return PIDNamespaceTranslationModeAuto
	}
	return cfg.PIDNamespaceTranslationMode
}
...
func loadRodataVars(coll *cebpf.CollectionSpec, kmod *kallsyms.Module, cfg *Config,
	...
	pidNamespaceMode := pidNamespaceTranslationMode(cfg)
	...
	if pidNamespaceTranslation {
		...
		if err := coll.Variables["pid_ns_translation_enabled"].Set(true); err != nil {
