...
//nolint:gocritic
//export __push_frame
func __push_frame(id, file, line C.u64, frameType C.uchar) C.int {
...
//nolint:gocritic
//export __bpf_probe_read
func __bpf_probe_read(id C.u64, buf unsafe.Pointer, sz C.int, ptr unsafe.Pointer) C.long {
	ctx := ebpfContextMap[id]
...
func __bpf_map_lookup_elem(id C.u64, mapdef *C.bpf_map_def, keyptr unsafe.Pointer) unsafe.Pointer {
	...
	case &C.interpreter_offsets, &C.perl_procs, &C.php_procs, &C.py_procs, &C.hotspot_procs,
		&C.ruby_procs, &C.v8_procs:
		var key any
