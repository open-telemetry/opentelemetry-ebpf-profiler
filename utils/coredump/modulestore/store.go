...
import (
	...
	zstpak "github.com/elastic/otel-profiling-agent/utils/zstpak/lib"
)
...
func (store *Store) InsertModuleLocally(localPath string) (id ID, isNew bool, err error) {
	...
	if err != nil {
		return ID{}, false, fmt.Errorf("failed to seek file back to start")
	}
...
func (store *Store) ListRemoteModules() (map[ID]time.Time, error) {
	...
	for _, object := range objectList {
		if object.Key == nil || object.LastModified == nil {
			return nil, fmt.Errorf("s3 object lacks required field")
		}
...
func (store *Store) ensurePresentLocally(id ID) (string, error) {
	...
	if !store.online {
		return "", fmt.Errorf("module store is operating in offline mode")
	}
