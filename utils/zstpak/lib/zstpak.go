...
func readFooter(input io.ReaderAt, fileSize uint64) (*footer, error) {
	...
	if fileSize < footerSize {
		return nil, fmt.Errorf("file is too small to be a valid zstpak file")
	}
	if _, err := input.ReadAt(buf[:], int64(fileSize-footerSize)); err != nil {
		return nil, fmt.Errorf("failed to read footer: %w", err)
	}
	if !bytes.Equal(buf[24:], []byte(magic)) {
		return nil, fmt.Errorf("file doesn't appear to be in zstpak format (bad magic)")
	}
	...
	if fileSize < footerSize+numberOfChunks*8 {
		return nil, fmt.Errorf("file too small to hold index table")
	}
	...
	for i := uint64(0); i < numberOfChunks; i++ {
		...
		if i > 0 && entry < index[i-1] {
			return nil, fmt.Errorf("index entries aren't monotonically increasing")
		}
...
func CompressInto(in io.Reader, out io.Writer, chunkSize uint64) error {
	...
	for {
		...
		compressed, err := zstd.Compress(compressBuf, readBuf)
		if err != nil {
...
func (reader *Reader) ReadAt(p []byte, off int64) (n int, err error) {
	...
	for remaining > 0 {
		...
		if skipOffset > len(decompressed) {
			return 0, fmt.Errorf("corrupted chunk data")
		}
...
func (reader *Reader) getDecompressedChunk(start, length uint64) ([]byte, error) {
	...
	decompressed, err := zstd.Decompress(nil, compressedChunk)
	if err != nil {
