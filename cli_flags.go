...
var (
	...
	reporterJitterHelp   = fmt.Sprintf(
		"Set the jitter applied to the reporter's interval as a fraction. "+
			"Valid values are in the range [0..1]. Default is %.1f.",
	...
	bpffsHelp = fmt.Sprintf("Set the root BPF FS path for pinned maps. "+
		"Only used for OBI span/trace ID communication. Default is %s",
		defaultBPFFSRoot)
	...
	pinnedCPUIDsHelp = "Range of CPUs to profile in the format like \"0-15,20,31\". " +
		...
		"(processes, IRQ handlers, etc.) are explicitly pinned to provided CPUs. " +
