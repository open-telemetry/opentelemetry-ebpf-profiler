...
func (g *Gopclntab) LookupSymbol(symbol libpf.SymbolName) (*libpf.Symbol, error) {
	...
	for i := 0; i < g.numFuncs; i++ {
		...
		name, _ := g.cache.UnsafeStringAt(g.funcnametab+int64(fun.nameOff), len(symbol))
		if name == symString {
...
func NewGopclntab(ef *pfelf.File) (*Gopclntab, error) {
	...
	case go1_2:
		...
		numSourceFiles := g.cache.Uint32At(int64(filetabOffset))
...
func (g *Gopclntab) Symbolize(pc uint64) (sourceFile libpf.String, line uint, funcName libpf.String) {
	...
	funcName, _ = g.cache.InternStringAt(int64(g.funcnametab) + int64(fun.nameOff))
	if fun.pcfileOff != 0 {
		if fileIndex, ok := g.mapPcval(fun.pcfileOff, uint(funcPc), uint(pc)); ok {
			if g.version >= go1_16 {
				fileIndex += fun.npcData
			}
			fileOffs := int64(g.cache.Uint32At(g.cutab + 4*int64(fileIndex)))
...
type sourceFileResolver func(sourceFile string, defaultStrategy strategy) strategy

type sourceStrategyResolver struct {
	g              *Gopclntab
	filetabCache   pfatbuf.Cache
	sourceStrategy map[int32]strategy
	resolver       sourceFileResolver
}

func newStrategyResolver(g *Gopclntab, resolver sourceFileResolver) *sourceStrategyResolver {
	ssr := &sourceStrategyResolver{
		g:              g,
		sourceStrategy: make(map[int32]strategy),
		resolver:       resolver,
	}
	ssr.filetabCache.Init(g.cache.Inner())
	return ssr
}

func getSourceFileStrategyAmd64(sourceFile string, defaultStrategy strategy) strategy {
...
// getFuncStrategy categorizes functions unwinding strategy based on its name
func (ssr *sourceStrategyResolver) resolve(funcPc uint64, fun *pclntabFunc, defaultStrategy strategy) strategy {
	fileStrategy, ok := ssr.sourceStrategy[fun.pcfileOff]
	if !ok {
		g := ssr.g
		p := g.getPcval(fun.pcfileOff, uint(funcPc))
		fileIndex := uint(p.val)
		if g.version >= go1_16 {
			fileIndex += uint(fun.npcData)
		}
		fileOffs := int64(g.cache.Uint32At(g.cutab + int64(4*fileIndex)))
		sourceFile, _ := ssr.filetabCache.UnsafeStringAt(g.filetab+fileOffs, 0)
		fileStrategy = ssr.resolver(sourceFile, defaultStrategy)
		ssr.sourceStrategy[fun.pcfileOff] = fileStrategy
	}
	return fileStrategy
}
...
func (ee *elfExtractor) parseGoPclntab() error {
	...
	var strategyResolver *sourceStrategyResolver

	switch arch {
	case elf.EM_X86_64:
		parsePclntab = parseX86pclntabFunc
		strategyResolver = newStrategyResolver(g, getSourceFileStrategyAmd64)
	case elf.EM_AARCH64:
	...
	cutabCache.Init(g.cache.Inner())
	funcNameCache := pfatbuf.Cache{}
	funcNameCache.Init(g.cache.Inner())
	...
	for i := 0; i < g.numFuncs; i++ {
		...
		funcName, _ := funcNameCache.UnsafeStringAt(g.funcnametab+int64(fun.nameOff), 32)
		...
		if fun.pcfileOff != 0 && strategyResolver != nil {
			fileStrategy = strategyResolver.resolve(funcPc, &fun, defaultStrategy)
		}
