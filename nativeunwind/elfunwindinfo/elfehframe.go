...
// newReaderFromProg
func newReaderFromProg(prog *pfelf.Prog, cache *pfatbuf.Cache) reader {
	cache.Init(prog)
	return reader{
		rd:    cache,
		vaddr: prog.Vaddr,
...
func (r *reader) str() string {
	str, err := r.rd.UnsafeStringAt(r.pos, 32)
	if err != nil {
		return ""
	}
...
func (r *reader) parseCIE(cie *cieInfo) (data reader, err error) {
	...
	*cie = cieInfo{
	...
	// Note: str returns unsafe string, but since we read only small consecutive
	// amount of bytes, the string returned can still be used in after few reads.
...
func (es *ehframeSections) locateSections(ef *pfelf.File) error {
	...
	es.header = newReaderFromProg(prog, &es.headerCache)
	es.frames = newReaderFromProg(prog, &es.framesCache)
...
func (ee *elfExtractor) walkBinSearchTable(ef *pfelf.File, es *ehframeSections) error {
	t, err := newEhFrameTableFromSections(es, ef.Machine)
	if err != nil {
		return err
	}
	r := t.entryAt(0)
	for f := uint64(0); f < t.fdeCount; f++ {
		ipStart, fr, err := t.decodeEntry(&r)
		if err != nil {
			return err
		}
		pos := fr.pos
		err = ee.parseFDE(&fr, ef, ipStart, t.cieCache, f > 0)
		if err != nil && err != errEmptyEntry {
			return fmt.Errorf("failed to parse FDE (%v/%v) %#x: %v", f, t.fdeCount, pos, err)
		}
	}
	return nil
}
...
func (ee *elfExtractor) walkFDEs(ef *pfelf.File, frames *reader) error {
	...
	for frames.hasData() {
		...
		if err != nil && err != errUnexpectedType && err != errEmptyEntry {
...
func (ee *elfExtractor) parseEHFrame() error {
	...
	if es.header.isValid() {
		...
		return ee.walkBinSearchTable(ee.file, &es)
	}

	// Otherwise, manually walk the FDEs.
	return ee.walkFDEs(ee.file, &es.frames)
}
