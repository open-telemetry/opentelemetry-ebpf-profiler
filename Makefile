...
support/golbls_1_23.test: generate ebpf
	CGO_ENABLED=0 GOTOOLCHAIN=go1.23.7 go test -C ./interpreter/golabels/integrationtests . -c -trimpath -tags $(GO_TAGS),nocgo,integration  -o $(PWD)/$@

support/golbls_1_24.test: generate ebpf
	CGO_ENABLED=0 GOTOOLCHAIN=go1.24.1 go test -C ./interpreter/golabels/integrationtests . -c -trimpath -tags $(GO_TAGS),nocgo,integration  -o $(PWD)/$@

support/golbls_cgo.test: generate ebpf
	CGO_ENABLED=1 GOTOOLCHAIN=go1.24.1 go test -C ./interpreter/golabels/integrationtests . -c -ldflags '-extldflags "-static"' -trimpath -tags $(GO_TAGS),withcgo,integration  -o $(PWD)/$@
