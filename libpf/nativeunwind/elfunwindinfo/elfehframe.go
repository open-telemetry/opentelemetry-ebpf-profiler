...
func (r *reader) expression() ([]dwarfExpression, error) {
	...
	data := r.bytes(blen)
	if data == nil {
		return nil, fmt.Errorf("expression data missing")
	}
	ed := reader{
		data: data,
		end:  blen,
...
func (r *reader) parseCIE(cie *cieInfo) error {
	...
	*cie = cieInfo{
	...
	if !r.isValid() {
		return fmt.Errorf("CIE not valid after header")
	}
	return err
}

// getUnwindInfo generates the needed unwind information from the register set
func (regs *vmRegs) getUnwindInfo() sdtypes.UnwindInfo {
	switch regs.arch {
	case elf.EM_AARCH64:
		return regs.getUnwindInfoARM()
	case elf.EM_X86_64:
		return regs.getUnwindInfoX86()
	default:
...
// http://dwarfstd.org/doc/DWARF5.pdf §6.4.1
// https://refspecs.linuxfoundation.org/LSB_5.0.0/LSB-Core-generic/LSB-Core-generic/ehframechpt.html
func (r *reader) parseFDE(ef, efCode *pfelf.File, ipStart uintptr, deltas *sdtypes.StackDeltaArray,
	hooks ehframeHooks, cieCache *lru.LRU[uint64, *cieInfo], sorted bool) (
	...
	if hooks != nil && !hooks.fdeHook(st.cie, &fde) {
		return uintptr(fde.len), nil
	}
	st.loc = fde.ipStart

	if st.cie.isSignalHandler || isSignalTrampoline(efCode, &fde) {
		...
		if hooks != nil {
			hooks.deltaHook(st.loc, &st.cur, delta)
		}
		deltas.AddEx(delta, sorted)
	} else {
		...
		for r.hasData() {
			...
			if hooks != nil {
				hooks.deltaHook(ip, &st.cur, delta)
			}
			deltas.AddEx(delta, sorted)
...
func findEhSections(ef *pfelf.File) (
	...
	if hdr := validateEhFrameHdr(ehFrameHdrSec); hdr == nil {
		...
		return nil, nil, fmt.Errorf("no suitable way to parsing eh_frame found")
	}
	...
	if len(data) == 0 {
		return nil, nil, fmt.Errorf("the eh_frame section is empty")
	}
...
func walkBinSearchTable(ef *pfelf.File, ehFrameHdrSec *elfRegion, ehFrameSec *elfRegion,
	deltas *sdtypes.StackDeltaArray, hooks ehframeHooks) error {
	h := (*ehFrameHdr)(unsafe.Pointer(&ehFrameHdrSec.data[0]))
	...
	for f := uintptr(0); f < fdeCount; f++ {
		...
		_, err = fr.parseFDE(ef, ef, ipStart, deltas, hooks, cieCache, true)
		if err != nil {
...
func walkFDEs(ef, efCode *pfelf.File, ehFrameSec *elfRegion, deltas *sdtypes.StackDeltaArray,
	hooks ehframeHooks, debugFrame bool) error {
	var err error
	...
	for f := uintptr(0); f < uintptr(len(ehFrameSec.data)); f += entryLen {
		...
		entryLen, err = fr.parseFDE(ef, efCode, 0, deltas, hooks, cieCache, false)
...
func parseEHFrame(ef *pfelf.File, deltas *sdtypes.StackDeltaArray, hooks ehframeHooks) error {
	...
	if ehFrameHdrSec != nil {
		...
		return walkBinSearchTable(ef, ehFrameHdrSec, ehFrameSec, deltas, hooks)
	}

	// Otherwise, manually walk the FDEs.
	return walkFDEs(ef, ef, ehFrameSec, deltas, hooks, false)
}

// parseDebugFrame parses the .debug_frame DWARF info, extracting stack deltas.
func parseDebugFrame(ef, efCode *pfelf.File, deltas *sdtypes.StackDeltaArray,
	hooks ehframeHooks) error {
	debugFrameSection := elfRegionFromSection(ef.Section(".debug_frame"))
	if debugFrameSection == nil {
		return nil
	}

	return walkFDEs(ef, efCode, debugFrameSection, deltas, hooks, true)
}
