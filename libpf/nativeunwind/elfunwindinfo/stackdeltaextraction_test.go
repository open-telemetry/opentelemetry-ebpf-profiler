...
func TestExtractStackDeltasFromFilename(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Failed to base64-decode the embedded executable?")
	}
	// Write the executable file to a temporary file, and the symbol
	// file, too.
	exeFile, err := os.CreateTemp("/tmp", "dwarf_extract_elf_")
	if err != nil {
		t.Errorf("failure to open tempfile")
	}
	defer exeFile.Close()
	if _, err = exeFile.Write(buffer); err != nil {
		t.Fatalf("failed to write buffer to file: %v", err)
	}
	if err = exeFile.Sync(); err != nil {
		t.Fatalf("failed to synchronize file: %v", err)
	}
	...
	if diff := cmp.Diff(data.Deltas[:len(firstDeltas)], firstDeltas); diff != "" {
		t.Errorf("Deltas are wrong: %s", diff)
	}
}
