...
#pragma unroll
  for (int i = 0; i < CODE_HEADER_SEARCH_ITERATIONS; i++) {
    DEBUG_PRINT("beam: CodeHeader search iteration %d", i);
    if (pc < current_range.start) {
      DEBUG_PRINT("beam: setting high to current");
      high = current;
    } else if (pc >= current_range.end) {
      DEBUG_PRINT("beam: setting low to current + 1");
      low = current + 1;
    } else {
    ...
    DEBUG_PRINT("beam: Let's check modules: %llx (n: %llu)", ranges->modules, ranges->n);
    if (ranges->modules == 0LL) {
      DEBUG_PRINT("beam: modules pointer not initialized: %llx (n: %llu)", ranges->modules, ranges->n);
      return -1;
    }
    if (!current) {
      DEBUG_PRINT("beam: current not initialized");
      return -1;
    }
    if (bpf_probe_read_user(&current_range, sizeof(BEAMRangeEntry), (void*)(ranges->modules + current * sizeof(BEAMRangeEntry)))) {
...
static EBPF_INLINE int unwind_beam(struct pt_regs *ctx) {
  ...
  if (bpf_probe_read_user(&ranges.last, sizeof(BEAMRangeEntry), (void*)(ranges.modules + ranges.n * sizeof(BEAMRangeEntry)))) {
