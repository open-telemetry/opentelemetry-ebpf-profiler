...
int tracepoint__sched_switch(UNUSED void *ctx)
{
  ...
  // Key the map on the (possibly translated) PID/TGID so the finish_task_switch
  // lookup below sees the same key the sched_switch entry stored.
...
{
  // Resolve PID/TGID in the profiler's PID namespace to match the key stored
  // by tracepoint__sched_switch above.
  u32 pid = 0;
  u32 tid = 0;
  if (!get_pid_tgid(&pid, &tid)) {
    return 0;
