			...
			tree: samples.TraceEventsTree{
				samples.ResourceKey{
					...
					Events: map[*samples.TypeMetadata]samples.SampleToEvents{
						profileTypeSampling: {
							{}: {
								Frames: singleFrameTrace(
									libpf.GoFrame, mapping, 0x10, "func1", libpf.NullString, 1),
				...
				samples.ResourceKey{
					PID:         2,
					ContainerID: libpf.Intern("container2"),
				}: samples.ResourceToProfiles{
					Events: map[*samples.TypeMetadata]samples.SampleToEvents{
						profileTypeSampling: {
							{}: {
								Frames: singleFrameTrace(
									libpf.GoFrame, mapping, 0x20, "func2", libpf.NullString, 2),
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < dic.StackTable().Len(); i++ {
				gotStackTable = append(gotStackTable,
					dic.StackTable().At(i).LocationIndices().AsRaw())
			}
