...
type TraceEventMeta struct {
	...
	Value          int64
	// AllocSize is the raw, un-weighted allocation size in bytes for heap
	// alloc events (see libpf.EbpfTrace.Size); combined with Value (the
	// byte-weighted estimator) it lets consumers derive an unbiased
	// object-count estimate. Zero/unused for all other origins.
	AllocSize int64
	PID, TID  libpf.PID
...
type TraceEvents struct {
	...
	// with Values. Only populated for heap alloc origins.
	AllocSizes []int64
}
