...
func TestCustomLabelHashIsOrderIndependent(t *testing.T) {
	...
	labelsC := map[libpf.String]libpf.String{
		libpf.Intern("first"):  libpf.Intern("two"),
		libpf.Intern("second"): libpf.Intern("one"),
	}
	require.NotEqual(t, hashCustomLabels(labelsA), hashCustomLabels(labelsC))
}
